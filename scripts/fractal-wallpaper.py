...
def sample_region(center_x, center_y, zoom, fractal_type, max_iter, julia_c_real, julia_c_imag, sample_size=25):
...
def find_interesting_region(fractal_type, max_iter, julia_c_real, julia_c_imag, depth=3):
    ...
    for level in range(depth):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                interest, frac_in, frac_out = sample_region(
                    test_x, test_y, zoom, fractal_type, max_iter,
        ...
        zoom_factor = random.uniform(2.5, 4.0) if level > 0 else random.uniform(2.0, 3.0)
        zoom *= zoom_factor
        
        print(f"  Depth {level + 1}/{depth}: interest={best_interest:.3f}, zoom={zoom:.2f}")
...
def main():
    ...
    parser.add_argument('--zoom', type=float, default=None, help='Zoom level (auto if not specified)')
    parser.add_argument('--center-x', type=float, default=None, help='Center X coordinate (auto if not specified)')
    parser.add_argument('--center-y', type=float, default=None, help='Center Y coordinate (auto if not specified)')
    ...
    parser.add_argument('--search-depth', type=int, default=3, help='Depth of recursive search for interesting regions')
    ...
    # Auto-find interesting region if coordinates not specified
    if args.zoom is None or args.center_x is None or args.center_y is None:
        print("Searching for interesting region...")
        center_x, center_y, zoom = find_interesting_region(
            args.type, args.iterations, args.julia_c_real, args.julia_c_imag,
